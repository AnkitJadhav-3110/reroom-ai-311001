...
serve(async (req) => {
  ...
  try {
    ...
    // Fail fast if Razorpay is not configured. Falling back to a
    // placeholder secret allowed forged HMAC signatures.
    ...
    console.error("Payment error details:", error instanceof Error ? error.message : error);
